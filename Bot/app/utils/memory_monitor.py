...
class MemoryMonitor:
    ...
    def check_memory_pressure(self) -> Dict[str, Any]:
        ...
        pressure_indicators = {
            ...
            "low_available_memory": current["available_mb"] < 512,  # Less than 512MB available
        ...
        if pressure_indicators["high_memory_usage"]:
            ...
            pressure_indicators["recommendation"].append("Enable model unloading after use")
        
        if pressure_indicators["low_available_memory"]:
            pressure_indicators["recommendation"].append("Increase chunk size to reduce total chunks")
...
def get_deployment_recommendations() -> Dict[str, Any]:
    ...
    if memory_gb < 0.5:  # Less than 512MB
        ...
        recommendations["platform_suitability"]["vercel"] = "Suitable with heavy optimization"
        recommendations["platform_suitability"]["railway"] = "Not recommended"
        recommendations["warnings"].append("Very low memory - consider serverless deployment")
    elif memory_gb < 1:  # Less than 1GB
        recommendations["platform_suitability"]["render_free"] = "Suitable with optimization"
        recommendations["platform_suitability"]["vercel"] = "Suitable"
        recommendations["platform_suitability"]["railway"] = "Suitable with optimization"
    else:  # 1GB or more
